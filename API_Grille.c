...
void Grille_init(int nbcase, int nbcl, int taille_pixel,Grille **G){
  int i;
  ...
  if ((taille_pixel*1.0)/nbcase>1){
    (*G)->cas_aff=0;
    (*G)->taille_case=taille_pixel/nbcase;
  }
  else{
    (*G)->cas_aff=1;
    (*G)->modulo=nbcase/taille_pixel;
  }
...
void Grille_chg_case(Grille *G, int i, int j, int c){
